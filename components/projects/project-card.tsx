...
export function ProjectCard({ idea, onSelect }: { idea: ProjectIdea; onSelect: () => void }) {
  ...
  return (
    <motion.div
      ...
      className="group relative overflow-hidden rounded-2xl border-2 border-black bg-white p-6 shadow-brutal transition-all hover:-translate-x-1 hover:-translate-y-1 hover:shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]"
    >
      <div className="flex items-start justify-between">
        ...
        <div className="flex h-12 w-12 items-center justify-center rounded-xl border-2 border-black bg-blue-400 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
      ...
      <div className="mt-6 space-y-4">
        <div>
          ...
          <div className="mt-1 h-3 overflow-hidden rounded-full border-2 border-black bg-neutral-100">
            <motion.div
              initial={{ width: 0 }}
        ...
        <div className="flex flex-wrap gap-2 text-[10px] font-bold">
          {idea.requiredComponents.slice(0, 4).map((comp, i) => (
            <span
              key={i}
              className={`flex items-center gap-1 rounded-md border-2 border-black px-2 py-1 ${
                comp.status === "In Stock" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"
              }`}
            >
          ...
          {idea.requiredComponents.length > 4 && (
            <span className="flex items-center gap-1 rounded-md border-2 border-black bg-neutral-100 px-2 py-1 text-neutral-600">
      ...
      <Button
        onClick={onSelect}
        className="mt-6 w-full border-2 border-black bg-black py-6 font-black text-white shadow-[4px_4px_0px_0px_rgba(255,255,255,0.2)] hover:bg-neutral-900"
      >
